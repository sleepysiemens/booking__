...
@section('content')
    ...
    <form class="container" id="app" method="post" action="{{route('admin.blog.update', $post->id)}}" enctype="multipart/form-data">
        @csrf
        @method('patch')
        <div class="row">
            <div class="card shadow border-0 rounded-3">
                <div class="card-body p-4">
                        ...
                        <div class="col-auto pe-0">
                            <select name="category" class="btn btn-light border-0 d-flex text-black">
                                @foreach($categories as $category)
                                    <option @if($category->id==$post->category) selected @endif value="{{$category->id}}">{{$category->name}}</option>
                                @endforeach
                            </select>
                        </div>
                    ...
                    <div class="row px-2 my-3">
                        <input type="text" name="title" class="col-12 input-append fw-500 fs-32px btn btn-light text-black text-start" value="{{$post->title}}" required>
                    </div>
                    ...
                    <div class="row px-2 mt-4">
                        <textarea class="col-12 input-append btn btn-light text-black text-start" rows="10" name="text" required>{{$post->text}}</textarea>
                    </div>
        ...
        <script>
            const preview_img="{{asset('img/blog/'.$post->image)}}";
